    ...
    $.each(templatedata.groups, function (a, g) {
        ...
        $.each(g.fields, function(b, f) {
            if (carddata.fields[f.name] != undefined && carddata.fields[f.name] != '') {
                ...
                if (f.control == "MULTI" && f.options) {
                    var c = 0;
                    $.each(value.split('|'), function(i, o) {
                        if (o != "" && f.options[o] != undefined && f.options[o] != "") {
                            field.append('<div class="row"> ' + f.options[o] + '</div>');
                            c++;
                        }
                    });
                ...
                } else if (f.control == "DROPDOWN" && f.options) {
                    fval = ((value != '' && value != undefined)?f.options[value]:'');
